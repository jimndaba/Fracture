...
void Fracture::DepthNode::execute(Renderer& renderer)
{
	...
	renderer.clear();
	AssetManager::getShader("DepthPass")->use();
	AssetManager::getShader("DepthPass")->setFloat("nearPlane", NearPlane);
	AssetManager::getShader("DepthPass")->setFloat("farPlane", FarPlane);
	...
	for (unsigned int i = 0; i < forwardRenderCommands.size(); ++i)
	{
		...
		renderer.Submit(command, AssetManager::getShader("DepthPass").get());
	}   
