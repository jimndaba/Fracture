...
Fracture::Environment::Environment(std::shared_ptr<Texture> environment, std::shared_ptr<Shader> shader):m_enviroment(environment)
{
   ...
   // ----------------------------------------------------------------------
    shader->use();
    shader->setTexture("equirectangularMap", environment.get(), 0);
   ...
   // --------------------------------------------------------------------------------
    ...
    glBindRenderbuffer(GL_RENDERBUFFER, captureRBO);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, 512, 512);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_bdrfTexture->id, 0);
